...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local[2]").setAppName("SimonPark")
  ...
  private def textContains(lang: String, textStack: String): Boolean = textStack.split(" ").contains(lang)

  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.filter(article => textContains(lang, article.text)).count().toInt
